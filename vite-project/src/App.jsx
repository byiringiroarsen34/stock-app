...
const [page, setPage] = useState("login"); // login | change
  ...
  const sellProduct = async (id) => {
    ...
    const quantity = Number(sellQty[id]);

    if (!authToken) return alert("Please log in first");
    if (!price || !quantity || quantity < 1) return alert("Enter valid price and quantity");
    ...
    try {
      ...
      const message = err.response?.data?.message || "Could not complete sale";
      alert(message);
    }
  };
